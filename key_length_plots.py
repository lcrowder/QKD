...
fig=plt.figure()
plt.plot(raw_keylengths,fsl_trad1[:,0])
plt.plot(raw_keylengths,fsl_sifted_decoy1[:,0])
plt.plot(raw_keylengths,fsl_raw_decoy1[:,0])
plt.legend(('Traditional BB84','Decoy BB84 (Bob checks sifted key)','Decoy BB84 (Bob checks raw key)'))
plt.yscale("log",base=2)
plt.xscale("log",base=2)
plt.xlabel('raw key length')
plt.ylabel('longest possible secret key length')
plt.title('Best Secret Key Length with 99% security (error rate='+str(error_rates[0])+')')
